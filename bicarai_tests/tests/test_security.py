...
class TestAuthSecurity:
    ...
    def test_TC_SEC_05_rbac_rater_hanya_lihat_sesi_sendiri(self, rater1_token, user_token):
        """TC-SEC-05: Rater dapat akses /sessions/recent tapi hanya melihat sesinya sendiri (bukan sesi user lain)."""
        # Rater adalah authenticated user — boleh akses endpoint, tapi data difilter per user_id
        resp = requests.get(
        ...
        if resp.status_code == 200:
            ...
            save = requests.post(
                ...
                headers=auth_headers(user_token),
                json={
                    "scenario": "Job Interview", "score_range": 3.0,
                    "score_accuracy": 3.0, "score_fluency": 3.0,
                    "score_coherence": 3.0, "score_phonology": 3.0, "duration_min": 2.0,
                }
            )
...
class TestSessionSecurity:
    ...
    def test_TC_SEC_06_user_tidak_bisa_akses_sesi_user_lain(self, user_token, admin_token):
        """TC-SEC-06: User tidak boleh melihat atau memodifikasi sesi milik user lain."""
    ...
    def test_TC_SEC_07_sql_injection_login(self):
        ...
        for payload in sql_payloads:
            ...
            assert resp.status_code != 500, \
                f"Server error 500 terdeteksi — kemungkinan unhandled injection"

    def test_TC_SEC_08_xss_pada_input_teks(self, user_token):
        """TC-SEC-08: Input XSS pada field teks tidak boleh dikembalikan tanpa di-escape."""
        xss_payload = "<script>alert('XSS')</script>"
        resp = requests.post(
            f"{BASE}/api/sessions",
            headers=auth_headers(user_token),
            json={
                "scenario":        xss_payload,   # XSS di field scenario
                "score_range":     3.0,
                "score_accuracy":  3.0,
                "score_fluency":   3.0,
                "score_coherence": 3.0,
