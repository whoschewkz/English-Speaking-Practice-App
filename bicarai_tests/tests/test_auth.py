...
# ============================================================
# TC-AUTH-01: Registrasi Pengguna Baru (Valid)
# ============================================================
class TestRegistrasi:
    ...
    def test_TC_AUTH_01_registrasi_valid(self):
        ...
        payload = {
            ...
            "email":    f"newuser_{int(time.time())}@bicarai.test",
    ...
    def test_TC_AUTH_02_registrasi_email_duplikat(self):
        """TC-AUTH-02: Registrasi dengan email yang sudah terdaftar harus ditolak."""
        payload = {
            "username": "dupuser",
            "email":    TEST_USERS["user"]["email"],  # Email yang sudah ada
            "password": "Duplicate@123"
        }
        resp = requests.post(f"{BASE}/api/auth/register", json=payload)
        print(f"\n[TC-AUTH-02] Status: {resp.status_code} | Body: {resp.text[:200]}")
        assert resp.status_code in (400, 409, 422), \
            f"Seharusnya ditolak untuk email duplikat, got {resp.status_code}"
...
# ============================================================
# TC-AUTH-04 s/d TC-AUTH-06: Login
# ============================================================
class TestLogin:

    def test_TC_AUTH_04_login_valid(self):
        """TC-AUTH-04: Login dengan kredensial valid harus mengembalikan JWT."""
        d = TEST_USERS["user"]
        # API menggunakan field 'username', bukan 'email' (LoginIn schema)
    ...
    def test_TC_AUTH_05_login_password_salah(self):
        ...
        # 429 = rate limit aktif (login gagal sebelumnya memicu throttle — tetap penolakan)
    ...
    def test_TC_AUTH_06_login_username_tidak_terdaftar(self):
        """TC-AUTH-06: Login dengan username tidak terdaftar harus ditolak (401) atau rate-limited (429)."""
        payload = {
        ...
        # 429 = rate limit aktif setelah beberapa percobaan gagal sebelumnya
...
class TestLogout:
    ...
    def test_TC_AUTH_08_logout_valid(self, user_token):
        ...
        if not fresh_token:
            pytest.skip("Tidak bisa dapatkan fresh token")
