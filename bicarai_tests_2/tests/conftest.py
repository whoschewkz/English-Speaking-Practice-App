...
# Kredensial admin yang di-seed otomatis oleh sistem
...
def login_user(username: str, password: str) -> dict:
    """Login dengan username (sesuai backend BicarAI LoginIn schema)."""
...
def setup_test_users():
    ...
    for role, data in TEST_USERS.items():
        ...
        if resp.status_code in (200, 201):
            ...
            print(f"  ⚠️  Gagal buat akun {role}: {resp.status_code} - {resp.text[:100]}")
...
def user_refresh_token():
    """
    Ambil refresh_token dari user login (session-scoped).
    Ada fallback 65 detik jika rate limit aktif.
    """
    d = TEST_USERS["user"]
    result = login_user(d["username"], d["password"])
